...
npm run -s xapps -- publish \
  --from apps/tenants/xconecta/xapps/guards/xconect-tenant-payment-policy-stripe-delegated/manifest.json \
  ...
  --replace __TENANT_PAYMENT_RETURN_SECRET_REF__=platform://payment:tenant:hmac?scope=client&scope_id=<xconecta-client-id> \
  --bump-version patch \
