...
import { setWidgetPlaceholder } from "./xconect-host-shell.js";
  ...
  return createReferenceMarketplaceRuntime({
    ...
    onStatePatch: (patch) => {
      if (patch && typeof patch === "object" && Object.keys(patch).length > 0) {
        console.debug("[xconect-host] widget state patch", patch);
      }
    },
    setWidgetPlaceholder,
    onExpandError: (error) => {
      console.warn("[xconect-host] failed to handle split-panel expand request", error);
    },
  });
}
