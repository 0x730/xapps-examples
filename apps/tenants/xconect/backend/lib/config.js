...
const explicitEnvPath = process.env.XCONECT_ENV_FILE || process.env.ENV_FILE;
...
export const PORT = Number(process.env.XCONECT_PORT || 3312);

export const GUARD_INGEST_API_KEY = String(
  process.env.XCONECT_GUARD_INGEST_API_KEY || "xconect-tenant-guard-dev-key",
);

export const TENANT_PAYMENT_URL = String(
  process.env.XCONECT_TENANT_PAYMENT_URL || "http://localhost:3312/tenant-payment.html",
)
  .trim()
  .replace(/\/+$/, "");

export const TENANT_PAYMENT_RETURN_SECRET = String(
  process.env.XCONECT_TENANT_PAYMENT_RETURN_SECRET || "",
).trim();

export const TENANT_PAYMENT_RETURN_SECRET_REF = String(
  process.env.XCONECT_TENANT_PAYMENT_RETURN_SECRET_REF || "",
).trim();

export const TENANT_PAYMENT_RETURN_URL_ALLOWLIST = String(
  process.env.XCONECT_TENANT_PAYMENT_RETURN_URL_ALLOWLIST ||
    "http://localhost:3312,http://127.0.0.1:3312",
).trim();

export const ALLOWED_ORIGINS = String(process.env.XCONECT_ALLOWED_ORIGINS || "").trim();
export const HOST_BOOTSTRAP_API_KEYS = String(
  process.env.XCONECT_HOST_BOOTSTRAP_API_KEYS ||
    process.env.XCONECT_TENANT_API_KEY ||
    process.env.XCONECT_GATEWAY_API_KEY ||
    process.env.XAPPS_API_KEY ||
    "",
).trim();
export const HOST_BOOTSTRAP_SIGNING_SECRET = String(
  process.env.XCONECT_HOST_BOOTSTRAP_SIGNING_SECRET || "",
).trim();

export const TENANT_SUBJECT_PROFILE_CANDIDATES_JSON = String(
  process.env.XCONECT_SUBJECT_PROFILE_CANDIDATES_JSON || "",
).trim();
...
export const GATEWAY_API_KEY = String(
  process.env.XCONECT_GATEWAY_API_KEY || process.env.XAPPS_API_KEY || "",
).trim();
...
export const TENANT_SEED_LOGO_FILE = path.resolve(backendDir, "public/xconect-seed-logo.svg");
