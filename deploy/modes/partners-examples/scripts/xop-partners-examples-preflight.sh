...
require_pair_if_any "XCONECTA_HOST_BOOTSTRAP_SIGNING_KEY_ID" "XCONECTA_HOST_BOOTSTRAP_VERIFIER_KEYS_JSON" "xconecta host bootstrap rotation"
if [[ -n "${XCONECTA_HOST_BOOTSTRAP_API_KEYS:-}${XCONECTA_HOST_BOOTSTRAP_SIGNING_SECRET:-}${XCONECTA_HOST_BOOTSTRAP_SIGNING_KEY_ID:-}${XCONECTA_HOST_BOOTSTRAP_VERIFIER_KEYS_JSON:-}${XCONECTA_HOST_SESSION_SIGNING_KEY_ID:-}${XCONECTA_HOST_SESSION_VERIFIER_KEYS_JSON:-}" ]]; then
  require_non_empty "XCONECTA_HOST_SESSION_SIGNING_SECRET"
fi
require_pair_if_any "XCONECTA_HOST_SESSION_SIGNING_KEY_ID" "XCONECTA_HOST_SESSION_VERIFIER_KEYS_JSON" "xconecta host session rotation"
...
if [[ "${ENABLE_XCONECTB:-0}" == "1" ]]; then
  ...
  require_pair_if_any "XCONECTB_HOST_BOOTSTRAP_SIGNING_KEY_ID" "XCONECTB_HOST_BOOTSTRAP_VERIFIER_KEYS_JSON" "xconectb host bootstrap rotation"
  if [[ -n "${XCONECTB_HOST_BOOTSTRAP_API_KEYS:-}${XCONECTB_HOST_BOOTSTRAP_SIGNING_SECRET:-}${XCONECTB_HOST_BOOTSTRAP_SIGNING_KEY_ID:-}${XCONECTB_HOST_BOOTSTRAP_VERIFIER_KEYS_JSON:-}${XCONECTB_HOST_SESSION_SIGNING_KEY_ID:-}${XCONECTB_HOST_SESSION_VERIFIER_KEYS_JSON:-}" ]]; then
    require_non_empty "XCONECTB_HOST_SESSION_SIGNING_SECRET"
  fi
  require_pair_if_any "XCONECTB_HOST_SESSION_SIGNING_KEY_ID" "XCONECTB_HOST_SESSION_VERIFIER_KEYS_JSON" "xconectb host session rotation"
...
if [[ "${ENABLE_XCONECTC:-0}" == "1" ]]; then
  ...
  require_pair_if_any "XCONECTC_HOST_BOOTSTRAP_SIGNING_KEY_ID" "XCONECTC_HOST_BOOTSTRAP_VERIFIER_KEYS_JSON" "xconectc host bootstrap rotation"
  if [[ -n "${XCONECTC_HOST_BOOTSTRAP_API_KEYS:-}${XCONECTC_HOST_BOOTSTRAP_SIGNING_SECRET:-}${XCONECTC_HOST_BOOTSTRAP_SIGNING_KEY_ID:-}${XCONECTC_HOST_BOOTSTRAP_VERIFIER_KEYS_JSON:-}${XCONECTC_HOST_SESSION_SIGNING_KEY_ID:-}${XCONECTC_HOST_SESSION_VERIFIER_KEYS_JSON:-}" ]]; then
    require_non_empty "XCONECTC_HOST_SESSION_SIGNING_SECRET"
  fi
  require_pair_if_any "XCONECTC_HOST_SESSION_SIGNING_KEY_ID" "XCONECTC_HOST_SESSION_VERIFIER_KEYS_JSON" "xconectc host session rotation"
